...
def analysisData(vulnerList):
    ...
    vulnerability_map = {
        ...
        "be declared external": "未检测的调用. 修复建议: 尽可能限制未检测的调用的使用.",
        "Detected issues with version pragma": "版本问题. 修复建议: 使用0.8.x以上的版本.",
        "external calls inside a loop": "油费限制. 修复建议: 尽可能限制外部调用的使用，尤其是在循环内.",
        "tx.origin for authorization": "tx.origin不安全使用. 修复建议: 始终使用 msg.sender 而不是 tx.origin 进行身份验证和授权. msg.sender准确地代表当前调用的直接发起者, 确保只有直接与您的合约交互的实体才能通过身份验证.",
        ...
        "uses delegatecall": "不安全的代理. 修复建议: 在使用 delegatecall 之前，请充分理解其含义。它在您的合约存储上下文中执行另一个合约的代码，这意味着被调用的合约可以更改您的合约的状态.",
        ...
        "allows anyone to destruct the contract": "不安全的自杀函数. 修复建议: 将调用自毁功能的能力限制为选定的一组地址，通常仅限于合约所有者或一组受信任的管理员。这可以通过修改器来实现，修改器根据授权地址列表检查调用者的地址.",
